    ...
    return (
      <div className="image-viewer-container flex flex-col items-center justify-center p-8 bg-surface rounded-xl border border-glass-border min-h-[420px] text-center">
        ...
        <div className="text-xs text-text-3 max-w-sm mt-1">
          Upload optical or SAR satellite imagery in the left panel to activate the high-resolution visualization canvas.
        </div>
      </div>
    );
  }
    ...
    >
      ...
      <div className="viewer-toolbar flex flex-wrap justify-between items-center px-3 py-2 bg-surface/85 backdrop-blur border-b border-glass-border z-20">
        ...
        <div className="flex items-center gap-1.5 text-xs font-mono">
          <button
            type="button"
            className="px-2 py-1 bg-surface-2 hover:bg-surface-3 rounded border border-glass-border text-text-2 hover:text-text"
          ...
          <button
            type="button"
            className="px-2 py-1 bg-surface-2 hover:bg-surface-3 rounded border border-glass-border text-text-2 hover:text-text text-[11px]"
            onClick={handleReset}
      ...
      >
        ...
        >
          {viewMode === 'before-after' && hasTwoImages ? (
            ...
            <div className="relative w-[560px] h-[360px] rounded-lg overflow-hidden border border-glass-border shadow-2xl bg-surface-2">
              {/* Image 1 (T1) */}
              ...
              <input
                ...
                className="absolute bottom-2 left-4 right-4 z-20 opacity-40 hover:opacity-100 transition-opacity"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 w-[720px] max-w-full">
              ...
              <div className="relative h-[340px] rounded-lg border border-glass-border overflow-hidden bg-gradient-to-br from-[#0c1626] to-[#12233f] flex flex-col justify-between p-3 shadow-xl">
                ...
                <div className="text-[10px] font-mono text-text-3 flex justify-between">
            ...
            <div className="relative w-[580px] h-[360px] rounded-lg border border-glass-border overflow-hidden bg-gradient-to-br from-[#0a121e] via-[#0f1d32] to-[#142642] flex flex-col justify-between p-4 shadow-2xl">
              ...
              {overlays.boundingBoxes &&
                bboxItems.map((ev, idx) => {
                  ...
                  // Normalize 1920x1080 to 580x360 canvas
                  const leftPct = (x1 / 1920) * 100;
                  const topPct = (y1 / 1080) * 100;
                  const widthPct = ((x2 - x1) / 1920) * 100;
                  const heightPct = ((y2 - y1) / 1080) * 100;
                  const isActive = activeEvidenceId === ev.evidence_id;
                  ...
                  return (
                    <div
                      ...
                      className={`absolute rounded transition-all pointer-events-none ${
                        isActive
                          ? 'border-2 border-cyan-300 bg-cyan-400/25 ring-4 ring-cyan-500/40'
                          : 'border border-cyan-400/80 bg-cyan-400/10'
                      }`}
                      style={{
                    ...
                    >
                      <span className="absolute -top-5 left-0 px-1 py-0.2 rounded bg-cyan-950 text-cyan-300 text-[9px] font-mono border border-cyan-700/50 whitespace-nowrap shadow">
                        {ev.claim.substring(0, 24)}... ({(ev.confidence * 100).toFixed(0)}%)
                      </span>
                    </div>
                  );
                })}
              ...
              <div className="flex flex-col items-center justify-center pointer-events-none opacity-40">
                ...
                <div className="font-mono text-xs text-text-3">{files[0].metadata?.sensor || 'Sentinel-2'} Multispectral Tile</div>
              </div>

              <div className="flex justify-between items-center text-[10px] font-mono text-text-3 z-10">
                <span>CRS: {files[0].metadata?.crs || 'EPSG:32643'}</span>
                <span>Bounds: [{files[0].metadata?.width || 1920} × {files[0].metadata?.height || 1080}]</span>
              </div>
            </div>
          )}
        </div>
